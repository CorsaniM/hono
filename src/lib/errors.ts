...
export function asTRPCError(error: unknown) {
  if (isTRPCClientError(error)) {
    const data = error.data as unknown as CustomErrorData; // Conversión a tipo intermedio
    return {
      data,
      cause: data.zodError?.message ?? "UNKNOWN",
      message: error.message,
      code: data.zodError ? "BAD_REQUEST" : "UNKNOWN",
    };
  }
  return null;
}
